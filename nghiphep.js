...
function findNV() {
  let searchNV = document.getElementById("searchTenNV").value;
  let check = false;
  var bangTKNV = document.getElementById("bangTKNV");

  for (let i in dsNghiPhep) {
    if (dsNghiPhep[i].ten.includes(searchNV)) {
      check = true;
      var delRowSearch = bangTKNV.deleteRow(1);
      var createRowSearch = bangTKNV.insertRow(1);
      var CellTen = createRowSearch.insertCell(0);
      CellTen.innerHTML = dsNghiPhep[i].ten;
      var CellMa = createRowSearch.insertCell(1);
      CellMa.innerHTML = dsNghiPhep[i].ma;
      var CellNamSinh = createRowSearch.insertCell(2);
      CellNamSinh.innerHTML = dsNghiPhep[i].namsinh;
      var CellNamLV = createRowSearch.insertCell(3);
      CellNamLV.innerHTML = dsNghiPhep[i].namLV;
      var CellGioiTinh = createRowSearch.insertCell(4);
      CellGioiTinh.innerHTML = dsNghiPhep[i].gioitinh;
      var CellNgayNP = createRowSearch.insertCell(5);
      CellNgayNP.innerHTML = dsNghiPhep[i].ngayNP;
    }
  }
  if (check == false) {
    alert("Không có nhân viên trùng với thông tin tìm kiếm");
    // var delRowSearch = bangTKNV.deleteRow(1);
  }
}
